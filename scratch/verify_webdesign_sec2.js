...
async function main() {
  ...
  try {
    ...
    for (const vp of viewports) {
      ...
      const sec2Info = await send('Runtime.evaluate', {
        expression: `
          (() => {
            const sections = Array.from(document.querySelectorAll('section'));
            const sec2 = sections[1];
            if (!sec2) return { error: 'Section 2 not found' };
            const h2 = sec2.querySelector('h2');
            const p = sec2.querySelector('p');
            const cards = Array.from(sec2.querySelectorAll('h3')).map(h3 => {
      ...
      console.log('[' + vp.name.toUpperCase() + '] Section 2 Info:', JSON.stringify(sec2Info.result.value, null, 2));

      // Capture screenshot
      const shot = await send('Page.captureScreenshot', { format: 'png' });
      const imgPath = path.join(artifactDir, 'verify_webdesign_sec2_' + vp.name + '.png');
