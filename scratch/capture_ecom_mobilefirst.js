...
async function main() {
  ...
  try {
    ...
    // Find section with MOBILE-FIRST COMMERCE
    await send('Runtime.evaluate', {
      expression: `
        const sections = Array.from(document.querySelectorAll('section'));
        const target = sections.find(s => s.textContent.includes('MOBILE-FIRST COMMERCE'));
        if (target) target.scrollIntoView({ behavior: 'instant', block: 'start' });
      `
    });
    await sleep(800);
    let scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_mobilefirst_desktop.png'), Buffer.from(scr.data, 'base64'));
    ...
    fs.writeFileSync(path.join(artifactDir, 'ecom_mobilefirst_tablet.png'), Buffer.from(scr.data, 'base64'));
    ...
    await sleep(800);
    scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_mobilefirst_mobile_top.png'), Buffer.from(scr.data, 'base64'));

    await send('Runtime.evaluate', { expression: `window.scrollBy(0, 350)` });
    await sleep(600);
    scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_mobilefirst_mobile_bottom.png'), Buffer.from(scr.data, 'base64'));
    ...
    console.log('MOBILE-FIRST SCREENSHOTS CAPTURED SUCCESSFULLY');
  } catch (err) {
