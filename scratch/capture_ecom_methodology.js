...
async function main() {
  ...
  try {
    ...
    // Scroll to section 3 (Methodology section)
    await send('Runtime.evaluate', {
      expression: `
        const sec = document.querySelectorAll('section')[3];
        if (sec) sec.scrollIntoView({ behavior: 'instant', block: 'start' });
      `
    });
    await sleep(800);
    let scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_methodology_desktop.png'), Buffer.from(scr.data, 'base64'));
    ...
    fs.writeFileSync(path.join(artifactDir, 'ecom_methodology_tablet.png'), Buffer.from(scr.data, 'base64'));
    ...
    await sleep(800);
    scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_methodology_mobile_top.png'), Buffer.from(scr.data, 'base64'));

    await send('Runtime.evaluate', { expression: `window.scrollBy(0, 450)` });
    await sleep(600);
    scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_methodology_mobile_bottom.png'), Buffer.from(scr.data, 'base64'));
    ...
    console.log('METHODOLOGY SCREENSHOTS CAPTURED SUCCESSFULLY');
  } catch (err) {
