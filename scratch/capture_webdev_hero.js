...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(artifactDir, 'webdev_hero_mobile_top.png'), Buffer.from(shotMobileTop.data, 'base64'));

    await send('Runtime.evaluate', { expression: `window.scrollBy(0, 480)` });
    await sleep(1000);
    const shotMobileBottom = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'webdev_hero_mobile_bottom.png'), Buffer.from(shotMobileBottom.data, 'base64'));
    ...
    fs.writeFileSync(path.join(artifactDir, 'webdev_hero_desktop.png'), Buffer.from(shotDesktop.data, 'base64'));

    console.log('Saved all webdev hero screenshots.');
    ws.close();
  } catch (e) {
    console.error(e);
  } finally {
