...
async function main() {
  ...
  try {
    ...
    // Scroll to section 2 (index 1 of sections)
    await send('Runtime.evaluate', {
      expression: `
        const sec = document.querySelectorAll('section')[1];
        sec ? sec.scrollIntoView({ behavior: 'instant', block: 'start' }) : null;
      `
    });
    await sleep(800);
    let scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_offerings_desktop.png'), Buffer.from(scr.data, 'base64'));
    ...
    fs.writeFileSync(path.join(artifactDir, 'ecom_offerings_tablet.png'), Buffer.from(scr.data, 'base64'));
    ...
    await sleep(800);
    scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_offerings_mobile_top.png'), Buffer.from(scr.data, 'base64'));

    // Scroll down inside section 2 on mobile to capture the rest of cards
    await send('Runtime.evaluate', {
      expression: `window.scrollBy(0, 450)`
    });
    await sleep(600);
    scr = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(path.join(artifactDir, 'ecom_offerings_mobile_bottom.png'), Buffer.from(scr.data, 'base64'));
