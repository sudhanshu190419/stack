...
async function capture() {
  ...
  try {
    ...
    // Scroll directly to contact / CTA section
    ...
    fs.writeFileSync(`${artifactDir}\\cta_mobile_current_top.png`, Buffer.from(shot1.data, 'base64'));

    await send('Runtime.evaluate', { expression: `window.scrollBy(0, 400)` });
    await sleep(500);
    const shot2 = await send('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync(`${artifactDir}\\cta_mobile_current_bottom.png`, Buffer.from(shot2.data, 'base64'));

    console.log('Saved CTA mobile screenshots');
  } finally {
    chrome.kill();
  }
}
