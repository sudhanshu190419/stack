...
async function main() {
  ...
  try {
    ...
    // Scroll through page and capture 5 snapshots
    for (let i = 0; i < 6; i++) {
      let scr = await send('Page.captureScreenshot', { format: 'png' });
      fs.writeFileSync(path.join(artifactDir, `work_mob_step_${i}.png`), Buffer.from(scr.data, 'base64'));
      await send('Runtime.evaluate', { expression: `window.scrollBy(0, 750)` });
      await sleep(500);
    }
    console.log('Done capturing mobile steps');
