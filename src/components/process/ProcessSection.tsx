...
export default function ProcessSection() {
  return (
    <section
      id="process"
      className="relative w-full overflow-hidden bg-[#FAF7F2] text-neutral-900 pt-16 sm:pt-20 lg:pt-28 pb-16 sm:pb-20 lg:pb-28 border-t border-black/[0.04]"
    >
      ...
      <div className="relative z-10 max-w-[1560px] mx-auto px-5 sm:px-8 lg:px-14 xl:px-16 2xl:px-20">
        {/* ========================================================================= */}
        {/* MOBILE & TABLET COMPOSITION (< lg) - DEDICATED VERTICAL STEPPER TIMELINE  */}
        {/* ========================================================================= */}
        <div className="block lg:hidden">
          {/* Mobile Top Bar: Category Label & Compact Flow Pill */}
          <div className="flex items-center justify-between gap-3 mb-6 sm:mb-8">
            <div className="flex items-center gap-2.5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#9E6941]" />
              <span className="text-[11px] sm:text-xs font-semibold tracking-[0.25em] uppercase text-neutral-700 select-none">
                OUR PROCESS
              </span>
            </div>

            <div className="flex items-center text-[9.5px] sm:text-[10.5px] font-semibold tracking-[0.16em] uppercase text-neutral-600 bg-black/[0.03] px-3 py-1 rounded-full border border-black/[0.05] select-none">
              <span>IDEAS</span>
              <span className="text-[#9E6941] mx-1.5 font-bold">→</span>
              <span>LAUNCH</span>
              <span className="text-[#9E6941] mx-1.5 font-bold">→</span>
              <span className="text-neutral-900 font-bold">SCALE</span>
            </div>
          </div>

          {/* Mobile Section Header */}
          <div className="text-left mb-8 sm:mb-10">
            <p className="text-[10.5px] sm:text-xs font-semibold tracking-[0.26em] uppercase text-neutral-500 mb-2">
              A STRUCTURED APPROACH
            </p>
            <h2 className="text-2xl sm:text-4xl font-bold text-neutral-950 tracking-tight leading-[1.12]">
              Built with a Plan,
              <br />
              Designed for <span className="text-[#9E6941]">Growth</span>
            </h2>
            <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed max-w-md mt-2.5 font-normal">
              From strategy to scaling, we follow a proven process to build
              websites that deliver real business results.
            </p>
          </div>

          {/* Connected Vertical Stepper Timeline */}
          <div className="relative pl-7 sm:pl-9">
            {/* Continuous Vertical Timeline Line running down the left */}
            <div
              className="absolute left-[13px] sm:left-[17px] top-4 bottom-7 w-[2px] pointer-events-none rounded-full"
              style={{
                background:
                  'linear-gradient(to bottom, #9E6941 0%, #D6CFC4 30%, #D6CFC4 80%, #9E6941 100%)',
              }}
            />
            ...
            <div className="space-y-6 sm:space-y-8">
              {PROCESS_STEPS.map((step, index) => (
                ...
                >
                  {/* Circular Milestone Node on the timeline */}
                  <div className="absolute -left-7 sm:-left-9 top-1.5 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-[#FAF7F2] border-2 border-[#9E6941] flex items-center justify-center shadow-xs z-10 transition-transform duration-200 group-hover:scale-105">
                    <span className="text-[11px] sm:text-xs font-bold font-mono text-[#9E6941] leading-none">
                      {step.number}
                    </span>
                  </div>

                  {/* Elevated Process Card */}
                  <div className="bg-white/85 backdrop-blur-xs border border-black/[0.06] rounded-2xl p-4 sm:p-5 shadow-2xs hover:shadow-xs transition-all duration-300">
                    {/* Card Header: Title & Tagline Pill */}
                    <div className="flex flex-wrap items-center justify-between gap-2 pb-1.5 border-b border-black/[0.03]">
                      <h3 className="text-base sm:text-lg font-bold text-neutral-950 tracking-tight">
                        {step.title}
                      </h3>
                      <span className="text-[9px] sm:text-[10px] font-semibold tracking-[0.16em] uppercase text-[#9E6941] bg-[#9E6941]/[0.08] px-2.5 py-0.5 rounded-full border border-[#9E6941]/15">
                        {step.tagline}
                      </span>
                    </div>

                    {/* Step Description */}
                    <p className="text-xs sm:text-[13px] leading-relaxed text-neutral-600 mt-2.5">
                      {step.description}
                    </p>

                    {/* Mockup Visual Presentation Container */}
                    <div className="relative mt-3.5 w-full h-[155px] sm:h-[180px] rounded-xl overflow-hidden bg-[#FAF7F2]/75 border border-black/[0.04] flex items-center justify-center p-2.5">
                      <div className="relative w-full h-full max-w-[320px] transition-transform duration-300 group-hover:scale-[1.02]">
                        <Image
                          src={step.imageSrc}
                          alt={step.imageAlt}
                          fill
            ...
            {/* Terminal Milestone Node at bottom of timeline */}
            <div className="relative mt-6 flex items-center gap-3 select-none">
              <div className="w-7 h-7 sm:w-8 sm:h-8 -ml-7 sm:-ml-9 rounded-full bg-[#9E6941] text-[#FAF7F2] flex items-center justify-center shadow-xs z-10">
                <span className="text-xs leading-none">✦</span>
              </div>
              <span className="text-[11px] sm:text-xs font-semibold tracking-[0.2em] uppercase text-neutral-500">
        ...
        {/* ========================================================================= */}
        {/* DESKTOP COMPOSITION (lg:block) - 100% PRESERVED EXACT ORIGINAL LAYOUT     */}
        {/* ========================================================================= */}
        <div className="hidden lg:block">
          ...
          <div className="relative w-full mb-8 sm:mb-10 lg:mb-12">
            ...
            <div className="flex absolute left-0 top-[52px] xl:top-[54px] items-center gap-3.5 z-10">
            ...
            <div className="flex absolute right-0 top-[52px] xl:top-[54px] items-center text-[10px] sm:text-[11px] font-semibold tracking-[0.22em] uppercase text-neutral-600 select-none z-10">
              <span>IDEAS</span>
          ...
          <div className="relative">
            ...
            <div className="absolute left-0 right-0 top-[213px] -translate-y-1/2 z-0 pointer-events-none">
            ...
            {/* 4 Step Columns Grid */}
            <div className="relative z-10 grid grid-cols-4 gap-6 xl:gap-8 items-start">
              {PROCESS_STEPS.map((step, index) => (
                <motion.div
                  key={step.number}
